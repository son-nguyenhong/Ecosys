"""
Audit Service — NFR-007, Banking-grade compliance
Writes to ppg_audit_log for all write operations.

Usage:
    await log_audit(db, entity_type, entity_id, action, changed_by, ...)
"""
...
async def log_audit(
    ...
    """
    Write one audit log entry.  Never raises — errors are swallowed so that
    a logging failure never aborts the business operation.
    """
    ...
    try:
        ...
        # NFR-005: audit failure must not crash the main service
        logger.error(
