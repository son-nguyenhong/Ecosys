"""
BA Tasks Router — ba_tasks table (reference schema)
...
router = APIRouter(prefix="/ba-tasks", tags=["ba-tasks"])

VALID_STATUSES = {"pending", "in_progress", "done", "blocked"}


class BaTaskCreate(BaseModel):
    project_id: str
    milestone_id: Optional[str] = None
    task_type: Optional[str] = "requirements"
...
async def create_ba_task(
    ...
    """, str(uuid4()), body.project_id, body.milestone_id,
        body.task_type or "requirements", body.title, body.description,
...
@router.put("/{task_id}")
async def update_ba_task(
    task_id: str,
    body: BaTaskUpdate,
    user: CurrentUser = None,
    db: asyncpg.Connection = Depends(get_db),
):
    existing = await db.fetchrow("SELECT id, status FROM ba_tasks WHERE id=$1", task_id)
    if not existing:
...
async def delete_ba_task(
    task_id: str,
    user: CurrentUser = None,
    db: asyncpg.Connection = Depends(get_db),
):
    result = await db.execute("DELETE FROM ba_tasks WHERE id=$1", task_id)
    if result == "DELETE 0":
        raise HTTPException(404, "Task not found")
